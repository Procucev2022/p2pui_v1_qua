...
describe('CancelPoComponent', () => {
  ...
  beforeEach(async () => {
    ...
    poService.cancelPoById.and.returnValue(of({ status: 'Success', message: 'ok' }));
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: MAT_DIALOG_DATA, useValue: { id: 'po1' } },
      ...
      .overrideTemplate(CancelPoComponent, '')
      .overrideComponent(CancelPoComponent, { set: { providers: [] } })
      .compileComponents();
  ...
  it('should warn when reason missing', () => {
    component.reason = null;
    component.cancelPo();
    expect(component.isInvalid).toBe(true);
    expect(toaster.warning).toHaveBeenCalled();
    expect(poService.cancelPoById).not.toHaveBeenCalled();
  });

  it('should cancel and close on Success', () => {
    component.reason = 'dup';
    component.cancelPo();
    expect(toaster.success).toHaveBeenCalled();
    expect(dialogRef.close).toHaveBeenCalledWith({ event: 'close' });
  });

  it('should not close when status is not Success', () => {
    poService.cancelPoById.and.returnValue(of({ status: 'Failure' }));
    component.reason = 'dup';
    component.cancelPo();
  ...
  it('focused real branch paths', () => {
    ...
    c.reason = null; try { c.cancelPo(); } catch (e) {}
    c.reason = 'x'; try { c.cancelPo(); } catch (e) {}
    ...
    try { c.cancelPo(); } catch (e) {}
    try { c.cancelPo(null); } catch (e) {}
    try { c.cancelPo(true); } catch (e) {}
    try { c.cancelPo(false); } catch (e) {}
    try { c.cancelPo({ id: '1', vendorId: 'v1', invalid: false, valid: true, value: { id: '1' }, status: 'Success', statusCode: 200, message: 'ok', target: { value: 'x', files: [] }, preventDefault() {}, stopPropagation() {} }); } catch (e) {}
