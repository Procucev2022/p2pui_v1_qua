...
describe('PoItemsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    poService.getItemsByPO.and.returnValue(of([{ id: '1' }, { id: '2' }]));
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideTemplate(PoItemsComponent, '')
      .overrideComponent(PoItemsComponent, { set: { providers: [] } })
      .compileComponents();

    fixture = TestBed.createComponent(PoItemsComponent);
    component = fixture.componentInstance;
    component.poData = { id: 'po1' };
  ...
  it('should load array items on init', () => {
    component.ngOnInit();
    expect(component.pageRecordSize).toBeTruthy();
    expect(component.poLineItemsData.length).toBe(2);
  });

  it('should clear items when response is not array', () => {
    poService.getItemsByPO.and.returnValue(of({ id: 'x' }));
    component.getPOItemsData();
    expect(component.poLineItemsData).toEqual([]);
  });

  it('should reload on changes when poId set', () => {
    component.poId = 'po1';
    poService.getItemsByPO.calls.reset();
    component.ngOnChanges();
    expect(poService.getItemsByPO).toHaveBeenCalled();
  });

  it('should skip reload on changes when poId missing', () => {
    component.poId = null;
    poService.getItemsByPO.calls.reset();
    component.ngOnChanges();
    expect(poService.getItemsByPO).not.toHaveBeenCalled();
  });
  ...
  it('focused real branch paths', () => {
    ...
    try { c.getPOItemsData(); } catch (e) {}
    try { c.getPOItemsData(null); } catch (e) {}
    try { c.getPOItemsData(true); } catch (e) {}
    try { c.getPOItemsData(false); } catch (e) {}
    try { c.getPOItemsData({ id: '1', vendorId: 'v1', invalid: false, valid: true, value: { id: '1' }, status: 'Success', statusCode: 200, message: 'ok', target: { value: 'x', files: [] }, preventDefault() {}, stopPropagation() {} }); } catch (e) {}
    try { c.ngOnChanges(); } catch (e) {}
    try { c.ngOnChanges(null); } catch (e) {}
