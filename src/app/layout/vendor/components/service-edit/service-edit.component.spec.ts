...
describe('ServiceEditComponent', () => {
  ...
  const dialogData = {
    rowData: { id: '1', name: 'svc' },
    vendorRegData: { vendorService: [{ id: '1' }, { id: '2' }] },
  };
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideTemplate(ServiceEditComponent, '')
      .overrideComponent(ServiceEditComponent, { set: { providers: [] } })
      .compileComponents();
  ...
  it('should return early on invalid form', () => {
    component.onEditService({ invalid: true, value: {} } as any);
    expect(vendorRegSer.editVendor).not.toHaveBeenCalled();
  });

  it('should toast success on edit', () => {
    component.onEditService({ invalid: false, value: { name: 'n' } } as any);
  ...
  it('should toast error when status not Success', () => {
    vendorRegSer.editVendor.and.returnValue(of({ status: 'Failure', message: 'bad' }));
    component.onEditService({ invalid: false, value: { name: 'n' } } as any);
  ...
  it('should iterate unmatched service ids without error', () => {
    (component as any).vendorServiceData = { id: 'missing' };
    component.onEditService({ invalid: false, value: {} } as any);
  ...
  it('focused real branch paths', () => {
    ...
    try { c.onEditService(); } catch (e) {}
    try { c.onEditService(null); } catch (e) {}
    try { c.onEditService(true); } catch (e) {}
    try { c.onEditService(false); } catch (e) {}
    try { c.onEditService({ id: '1', vendorId: 'v1', invalid: false, valid: true, value: { id: '1' }, status: 'Success', statusCode: 200, message: 'ok', target: { value: 'x', files: [] }, preventDefault() {}, stopPropagation() {} }); } catch (e) {}
