...
describe('VendorContactsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [CommonModule, VendorContactsComponent],
      providers: [
        ...
        { provide: MatDialogRef, useValue: dialogRef },
        {
          ...
          useValue: { action: 'Add', productName: 'P1' },
        },
      ],
      schemas: [NO_ERRORS_SCHEMA],
    })
      .overrideTemplate(VendorContactsComponent, '')
      .overrideComponent(VendorContactsComponent, { set: { providers: [] } })
      .compileComponents();
  ...
  it('should copy dialog data in constructor', () => {
    expect(component.action).toBe('Add');
    expect(component.local_data).toEqual(
      jasmine.objectContaining({ action: 'Add', productName: 'P1' })
    );
  });
  ...
  it('should return early when add form is invalid', () => {
    component.onAddContact({ invalid: true, value: { name: 'x' } });
    expect(dialogRef.close).not.toHaveBeenCalled();
  });

  it('should submit valid contact form', () => {
    component.onAddContact({ invalid: false, value: { name: 'Alice' } });
  ...
  it('pattern-branch coverage', () => {
    ...
    const invalidForm = { invalid: true, valid: false, value: {}, form: { valid: false } };
    const validForm = { invalid: false, valid: true, value: { name: 'n' }, form: { valid: true } };
    try { c.onAddContact(invalidForm); } catch (e) {}
    try { c.onAddContact(validForm); } catch (e) {}
    try { c.submit(invalidForm); } catch (e) {}
    try { c.submit(validForm); } catch (e) {}
    ...
    try { c.onAddContact({ invalid: false, valid: true, value: { id: '1' }, form: { valid: true }, id: '1', status: 'Success', message: 'ok', target: { value: 'x', files: [], checked: true }, preventDefault() {}, stopPropagation() {} }); } catch (e) {}
    try { c.onAddContact({ invalid: true, valid: false, value: {}, form: { valid: false } }); } catch (e) {}
  ...
  it('focused real branch paths', () => {
    ...
    try { c.onAddContact(true); } catch (e) {}
    try { c.onAddContact(false); } catch (e) {}
    try { c.onAddContact({ id: '1', vendorId: 'v1', invalid: false, valid: true, value: { id: '1' }, status: 'Success', statusCode: 200, message: 'ok', target: { value: 'x', files: [] }, preventDefault() {}, stopPropagation() {} }); } catch (e) {}
