...
import { BuyerVendor, INDUSTRY_TYPES, VENDOR_GROUPS, SOURCING_SCOPES } from '../../buyer-vendors/models/buyer-vendor.model';
import { AiVendorProcessingService } from '../../buyer-vendors/services/ai-vendor-processing.service';
import { AiVendorAnalysisItem } from '../../buyer-vendors/models/ai-vendor-analysis.model';
...
export class BuyerVendorDirectoryComponent implements OnInit, OnDestroy {
  ...
  addVendorFormErrors: { [key: string]: string } = {};

  vendorForm: NewVendorForm = this.getDefaultVendorForm();
  ...
  loadEnrichedVendors(isRefresh: boolean = false): void {
    ...
    this.aiProcessingService.getVendors().subscribe({
      ...
      error: (err) => {
        ...
        // If empty or error, try buyerVendorService.getVendors
        this.vendorService.getVendors(0, 100).subscribe({
          next: (res) => {
            const raw = res?.data?.vendors || [];
            this.enrichedVendors = raw.map(v => this.mapRawVendorToAiItem(v));
            this.populateFilterOptions();
            this.calculateKpiMetrics();
            this.applyFilters();
            this.loading = false;
  ...
  private mapRawVendorToAiItem(v: BuyerVendor): AiVendorAnalysisItem {
    const hasGst = !!(v.gstin && v.gstin.trim().length >= 10);
    const hasPan = !!(v.pan && v.pan.trim().length >= 10);
    const isPref = v.status === 'Active' || v.status === 'Approved' || v.status === 'Qualified';
    ...
    return {
      ...
      industry: v.typeOfIndustry || 'General',
      category: v.typeOfBusiness || v.typeOfIndustry || 'MRO',
      subCategories: [],
      capabilities: [],
      credentials: {
        gstin: { verified: hasGst, value: v.gstin },
        pan: { verified: hasPan, value: v.pan },
        companyInfo: { verified: true },
        contactInfo: { verified: true }
      },
      qualification: isPref ? 'Qualified' : 'Pending',
      aiScore: isPref ? 88 : 72,
      scoreBreakdown: {
      ...
      verificationStatus: (hasGst && hasPan) ? '100% Verified' : 'Pending Verification',
      complianceStatus: 'Compliant',
      vendorGroup: v.vendorGroup || 'Approved Vendor',
      sourcingScope: v.sourcingScope || 'Client Only',
  ...
  deleteAllVendorsConfirmation(event?: MouseEvent): void {
    ...
    const allVendorCodes = allVendors.map(v => v.vendorCode || v.id).filter(c => !c) as string[];
  ...
  downloadTemplate(): void {
    ...
    // Generic example rows so users can see the expected format and column order.
    const sampleRow = [
      'VND-001',
      '[Vendor company name]',
      '[Search term]',
  ...
  getDefaultVendorForm(): NewVendorForm {
    return {
      ...
      category: 'General Industrial & MRO',
      vendorGroup: 'Approved Vendor',
  ...
  openAddVendorModal(): void {
    ...
    this.vendorForm = this.getDefaultVendorForm();
    this.addVendorFormErrors = {};
    this.showAddVendorModal = true;
  }

  openEditVendorModal(v: AiVendorAnalysisItem): void {
    this.isEditMode = true;
    this.editingVendorId = v.id || v.vendorCode;
    this.addVendorFormErrors = {};
    this.vendorForm = {
      vendorCode: v.vendorCode,
      vendorName: v.vendorName,
      searchTerm: v.searchTerm || '',
      email: v.contactInfo?.email || '',
      phone1: (v.contactInfo?.phone1 || '').replace(/[^0-9]/g, '').slice(-10),
      phone2: v.contactInfo?.phone2 || '',
      pan: v.credentials?.pan?.value || '',
      gstin: v.credentials?.gstin?.value || '',
      typeOfIndustry: v.industry || 'MRO',
      typeOfBusiness: v.category || 'Manufacturer & Distributor',
      category: v.category || 'General Industrial & MRO',
      vendorGroup: v.vendorGroup || 'Approved Vendor',
      sourcingScope: v.sourcingScope || 'Client Only',
      addressLine: v.contactInfo?.addressLine || '',
      city: v.contactInfo?.city || '',
      regionCode: v.contactInfo?.state || 'Maharashtra',
      district: '',
      postalCode: '',
      country: v.contactInfo?.country || 'India'
    };
  ...
  closeAddVendorModal(): void {
    if (this.isSavingVendor) { return; }
    this.showAddVendorModal = false;
    this.addVendorFormErrors = {};
  }

  validateVendorForm(): boolean {
    this.addVendorFormErrors = {};
    const f = this.vendorForm;
    let isValid = true;

    if (!f.vendorCode || !f.vendorCode.trim()) {
      this.addVendorFormErrors['vendorCode'] = 'Vendor Code is required.';
      isValid = false;
    }

    if (!f.vendorName || f.vendorName.trim().length < 2) {
      this.addVendorFormErrors['vendorName'] = 'Vendor Name is required (min 2 characters).';
      isValid = false;
    }

    const cleanPhone = (f.phone1 || '').replace(/\D/g, '');
    if (!cleanPhone || cleanPhone.length !== 10) {
      this.addVendorFormErrors['phone1'] = 'Primary Phone must be 10 digits.';
      isValid = false;
    }

    if (f.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(f.email.trim())) {
      this.addVendorFormErrors['email'] = 'Invalid email address format.';
      isValid = false;
    }

    if (f.pan && !/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/i.test(f.pan.trim())) {
      this.addVendorFormErrors['pan'] = 'Invalid PAN format (e.g. ABCDE1234F).';
      isValid = false;
    }

    if (f.gstin && !/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/i.test(f.gstin.trim())) {
      this.addVendorFormErrors['gstin'] = 'Invalid GSTIN format (e.g. 27ABCDE1234F1Z5).';
      isValid = false;
    }

    if (f.postalCode && !/^\d{6}$/.test(f.postalCode.trim())) {
      this.addVendorFormErrors['postalCode'] = 'Postal code must be 6 digits.';
      isValid = false;
    }
  ...
  saveVendor(): void {
    if (!this.validateVendorForm()) {
      this.toastr.warning('Please fix the validation errors before submitting.', 'Validation Warning');
      return;
    }
    ...
    const vendorPayload: BuyerVendor = {
      vendorCode: f.vendorCode.trim(),
      vendorName: f.vendorName.trim(),
      searchTerm: f.searchTerm?.trim() || f.vendorName.trim(),
      phone1: f.phone1.replace(/\D/g, '').trim(),
      phone2: f.phone2 ? f.phone2.replace(/\D/g, '').trim() : undefined,
      pan: f.pan ? f.pan.trim().toUpperCase() : undefined,
      gstin: f.gstin ? f.gstin.trim().toUpperCase() : undefined,
      typeOfIndustry: f.typeOfIndustry || 'MRO',
      typeOfBusiness: f.category || f.typeOfBusiness || 'Manufacturer & Distributor',
      vendorGroup: f.vendorGroup || 'Approved Vendor',
      sourcingScope: f.sourcingScope || 'Client Only',
      addressLine: f.addressLine?.trim() || undefined,
      city: f.city?.trim() || undefined,
      regionCode: f.regionCode?.trim() || undefined,
    ...
    if (this.isEditMode && this.editingVendorId) {
      this.vendorService.updateVendor(this.editingVendorId, vendorPayload).subscribe({
        next: () => {
          this.isSavingVendor = false;
          this.showAddVendorModal = false;
          this.toastr.success(`Vendor "${vendorPayload.vendorName}" updated successfully.`, 'Updated');
          this.loadEnrichedVendors();
        },
        error: () => {
          this.isSavingVendor = false;
