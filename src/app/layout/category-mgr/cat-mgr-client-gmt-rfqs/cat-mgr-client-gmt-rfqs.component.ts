...
export class CatMgrClientGmtRfqsComponent implements OnInit {
    ...
    getRFQsByCategoryManagerForGlobalSearch(){
           this.rfqservice.getAllClientRFQsByGMTForCMandCM2ByGlobalSearch(this.searchBy, this.searchTextValue).subscribe((res: any) => {
    ...
    getRfqsByCategoryManager(startPage: number = 0, pageSize: number = 100) {
        this.selectedData = [];

        this.rfqservice.getAllClientRFQsByGMTForCMandCM2ByPagination(startPage, pageSize).subscribe((res: any) => {
            if (Array.isArray(res.data)) {
                const rfqDataList = res.data.map((ele: any) => {
                ...
                this.rfqDataList = [...rfqDataList];
                this.cache_rfqDataList = [...rfqDataList];
                this.totalRecords = res.totalRecords || 0;
            } else {
                this.rfqDataList = [];
                this.cache_rfqDataList = [];
                this.totalRecords = 0;
            }
        });
    }
