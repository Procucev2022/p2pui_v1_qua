...
@Component({
  ...
  templateUrl: './create-rfq-shared.component.html',
...
export class CreateRFQSharedComponent implements OnInit , OnChanges {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        this.buildRFQForms();
        if(changes && changes.rfqDetails.currentValue){
            this.onSendRFQ(changes.rfqDetails.currentValue)
        }
    }
    ...
    getRFQsForClient(){
        ...
        this.rfqservice.getAllRFQsForNoPRForClientInitiatorGMT(obj).subscribe(data => { this.rfqDataList = data || []; });
    }
        ...
        this.createRFQService.forwardRFQ(obj).subscribe((res: any) => {
            if (res.status == 'Success') {
                ...
                this.toaster.success(res.message, 'Success');
                    this.onCloseRFQForwardScreen.emit(true)
            } else {
