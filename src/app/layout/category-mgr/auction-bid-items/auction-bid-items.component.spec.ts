...
describe('AuctionBidItemsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideTemplate(AuctionBidItemsComponent, '')
      .overrideComponent(AuctionBidItemsComponent, { set: { providers: [] } })
      .compileComponents();
  ...
  it('should default empty list when auction items missing', () => {
    component.auctionBIdItems = null;
    component.ngOnInit();
    expect(component.bidItemsList).toEqual([]);
    expect(component.pageRecordSize).toBeTruthy();
  });

  it('should use provided auction items', () => {
    component.auctionBIdItems = [{ id: 1 }];
    component.getLineItems();
    expect(component.bidItemsList.length).toBe(1);
  });

  it('should reload on changes when bidId set', () => {
    component.auctionBIdItems = [{ id: 2 }];
    component.bidId = 'b1';
    component.ngOnChanges();
    expect(component.bidItemsList).toEqual([{ id: 2 }]);
  });

  it('should skip reload when bidId missing', () => {
    component.bidItemsList = [{ keep: true }];
    component.bidId = null;
    component.ngOnChanges();
    expect(component.bidItemsList).toEqual([{ keep: true }]);
  });

  it('should capture page event', () => {
