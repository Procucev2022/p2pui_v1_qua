...
describe('CatMgrQuotSubTabRfqsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    quotService.getRFQsByQuot.and.returnValue(of({ id: 'rfq1', rfqStatus: 'Open' }));
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideTemplate(CatMgrQuotSubTabRfqsComponent, '')
      .overrideComponent(CatMgrQuotSubTabRfqsComponent, { set: { providers: [] } })
      .compileComponents();
  ...
  it('should load rfqs when response has id', () => {
    component.ngOnInit();
    expect(component.lineItemsList.length).toBe(1);
    expect(component.pageRecordSize).toBeTruthy();
  });

  it('should not set list when response lacks id', () => {
    component.lineItemsList = [];
    quotService.getRFQsByQuot.and.returnValue(of({ status: 'ok' }));
    component.getRfqs();
    expect(component.lineItemsList).toEqual([]);
  });

  it('should reload on changes when quotData.id present', () => {
    quotService.getRFQsByQuot.calls.reset();
    component.ngOnChanges();
    expect(quotService.getRFQsByQuot).toHaveBeenCalled();
  });
  ...
  it('should skip reload when quotData.id missing', () => {
    ...
    quotService.getRFQsByQuot.calls.reset();
    component.ngOnChanges();
    expect(quotService.getRFQsByQuot).not.toHaveBeenCalled();
  });
