...
describe('CatMgrQuotSubTabLineItemsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    quotService.getLineItemsByQuot.and.returnValue(of([{ id: '1' }]));
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideTemplate(CatMgrQuotSubTabLineItemsComponent, '')
      .overrideComponent(CatMgrQuotSubTabLineItemsComponent, { set: { providers: [] } })
      .compileComponents();
  ...
  it('should load array line items on init', () => {
    component.ngOnInit();
    expect(component.lineItemsList.length).toBe(1);
  });

  it('should clear when response is not array', () => {
    quotService.getLineItemsByQuot.and.returnValue(of({ id: 'x' }));
    component.getLineItems();
    expect(component.lineItemsList).toEqual([]);
  });

  it('should reload on changes when quotData.id present', () => {
    quotService.getLineItemsByQuot.calls.reset();
    component.ngOnChanges();
    expect(quotService.getLineItemsByQuot).toHaveBeenCalled();
  });

  it('should skip reload when quotData.id missing', () => {
    component.quotData = {};
    quotService.getLineItemsByQuot.calls.reset();
    component.ngOnChanges();
    expect(quotService.getLineItemsByQuot).not.toHaveBeenCalled();
  });
