...
@Component({
    selector: 'app-client-procure-request-capex',
    templateUrl: './client-procure-request-capex.component.html',
    styleUrls: ['./client-procure-request-capex.component.scss'],
    animations: [
...
export class ClientProcureRequestCapexComponent implements OnInit, OnDestroy {
    ...
    prSummaryHeaders: any = [
        ...
        { field: 'status', header: 'Status', isLink: false , width: '175px' , fieldType: 'text', head: 'Status', isExceedContent: false },
    ...
    onSelectStatus(e) {
        ...
        if (e.code !== 'All') {
            ...
            this.clientService.getPRByClientStatus(obj).subscribe((data:any) => {
                if(Array.isArray(data)){
                    this.prSummaryList = data.map((ele:any)=>{
                        return {...ele, status: ele.clientStatus?  ele.clientStatus.uiDisplay: (ele.procucevStatus? ele.procucevStatus.uiDisplay: '')}
                    })
                }
    ...
    getPrSummaryData() {
        ...
        if (this.loggedUserPermissions.includes('PC_C_PR_PAGE_APPROVE')) {
            ...
            if(this.loggedUserDetails.role.roleName &&  [ 'CategoryManager2',  'CategoryManager'].includes( this.loggedUserDetails.role.roleName) ){
                url = AppApiConfig.GET_ALL_PRS_BY_ORGID_AND_STATUS;
                req ={
                    "masterStatus": [
                        "PC_PR_NEW",
                        "PC_PR_ACCEPTED",
        ...
        this.clientService
            ...
            .subscribe(data => {
                ...
                this.prSummaryList = Array.isArray(data)? data.filter(ele => ele.capexFlag == true).map((ele:any)=>{
                    return {...ele, status: ele.clientStatus ?  ele.clientStatus.uiDisplay: (ele.procucevStatus? ele.procucevStatus.uiDisplay: '')}
    ...
    viewPrByIdModal() {
        const dialogConfig = new MatDialogConfig();
        dialogConfig.autoFocus = true;
        dialogConfig.data = {...this.viewPrByIdData, isCapex: true};
